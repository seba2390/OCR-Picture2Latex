...
We also perform experiments using fraternal dropout with a grid search on all the hyper-parameters and find that it leads to further improvements in performance. The details of this experiment can be found in section \ref{FDvsELD}. %However, our goal here is to rule out the possibility of outperforming just because of using better hyper-parameters.
...
% However, we outperform the best WT2 result by a larger margin than in the case of PTB. It may mean that the community has found better hyper-parameters for PTB in general and direct applications of new methods on top of existing models are harder.
...
We argue that in this task smaller $\kappa$ values are optimal because the image captioning encoder is given all information in the beginning and hence the variance of consecutive predictions is smaller that in unconditioned natural language processing tasks. \emph{Fraternal dropout} may benefits here mainly due to averaging gradients for different mask and hence updating weights more frequently.%\textit{[Since Adam optimizer was used, it may be more important not to feed its statistics with misleading zeros.]}

%\subsection{Language translation task}

%In the case of language translation, we apply fraternal dropout on multimodal machine translation task. We test out our model on the Multi30K dataset, a multilingual English-German image description dataset, which is used as a benchmark for WMT16 multimodal machine translation. 
...
%\hline
%dropout applied to layers & $U(0.2,0.6)$\\
%dropout for rnn layers & $U(0.15,0.45)$\\
%dropout for input embedding layers & $U(0.2,0.6)$\\
%dropout to remove words from embedding layer & $U(0.05,0.15)$\\
%weight dropout to apply to the RNN hidden to hidden matrix & $U(0.25,0.75)$\\
\vspace{-0.08cm}
\end{tabular}
