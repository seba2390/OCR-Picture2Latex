...
We study the problem of communicating a binary memoryless source $\{S(t)\}_{t=1,2, \ldots}$ to three users over an erasure broadcast channel with causal feedback available to the transmitter as well as all receivers.  %Let $\mathcal{S}^{N}$ be the set of all $N$-vectors with components in $\mathcal{S}$, and denote $(S(1), S(2), \ldots , S(N))$ as $S^{N}$.  For convenience, we will also denote the set $\{1,2,\dots,N\}$ as $[N]$.
%
%\begin{figure}
%	\centering
