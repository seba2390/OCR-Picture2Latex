...
%The general problem we consider involves communicating a binary memoryless source $\{S(t)\}_{t=1,2, \ldots}$ to $n$ users over an erasure broadcast channel with feedback.  %Let $\mathcal{S}^{N}$ be the set of all $N$-vectors with components in $\mathcal{S}$, and denote $(S(1), S(2), \ldots , S(N))$ as $S^{N}$.  For convenience, we will also denote the set $\{1,2,\dots,N\}$ as $[N]$.
%%
%%\begin{figure}
%%	\centering
...
%
%Let $Y_{i}(t)$ be the channel output observed by user $i$ on the $t^{\mathrm{th}}$ channel use for $i \in [n] \triangleq \{1, 2, \ldots, n\}$. %$\mathcal{U}= \{1, 2, 3\}$ and $t \in [W] = \{1, \ldots, W\}$.  %Our channel model is a binary erasure broadcast channel. % as shown in Fig.~\ref{fig:schematic}.  
...
%\begin{mydef}
%\label{def:code}
%	An $(N, W, d_{1}, d_{2}, \ldots, d_{n})$ code for source $S$ on the erasure broadcast channel consists of	
%	\begin{enumerate}
...
%		
%		\item $n$ decoding functions $g_{i,N} : \mathcal{Y}^{W} \times  \prod_{j = 1}^{n} \mathcal{Z}^{W} \to \mathcal{\hat{S}}^{N}$ s.t.\ $\hat{S}_{i}^{N} = g_{i,N}(Y_{i}^{W}, Z_1^{W}, Z_2^{W}, \ldots, Z_n^{W})$, $i \in [n]$,
%		\begin{enumerate}
