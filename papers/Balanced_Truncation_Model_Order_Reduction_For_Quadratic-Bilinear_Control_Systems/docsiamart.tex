...
\url{http://www.siam.org/journals/auth-info.php}. The files that
are distributed for the standard macros are given below. 
\begin{itemize}
\item \texttt{siamart0516.cls} (required): Main SIAM standard \LaTeX\ class file.
...
\end{itemize}
To use these files, put \texttt{siamart0516.cls} and
...
The \href{http://www.siam.org}{SIAM homepage} has general information.
...
the location explicitly instead by using \url{http://www.siam.org}.
\end{example}
...
\documentclass[supplement]{siamart0516}
\input{ex_shared}
...
Authors are required to provide DOIs; if they are not provided, they
...
\begin{itemize}
\item Change in file names: \texttt{siamltex1213.cls} is replaced by
  \texttt{siamart0516.cls}, and the \texttt{siam.bst} bibliography style
...
\subsection{Labeling objects according to the section number}
SIAM recommends numbering  objects sequentially. 
However,
if you prefer 
to number objects by section, e.g., Theorem 5.1
would indicate the first theorem in Section 5, put the code from
\cref{ex:numbering} into the document preamble.
