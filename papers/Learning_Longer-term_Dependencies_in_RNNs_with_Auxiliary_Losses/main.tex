...
%The reconstruction loss improves the memory of the recurrent network such that the number of steps needed for classification backprop is also small.}
\label{fig:method}
\end{figure*}
...
%Although we focus on very long input time scales, our experiments include datasets of shorter sequences (less than 1,000 steps) before gradually moving on to much longer ones. 
% To ensure complex dependencies are presented across the input sequence, image classification tasks are used where pixels of each image are fed into the recurrent models sequentially. 
%Flattening MNIST images into pixel sequences is first suggested by \citet{le2015simple} and has become the most popular benchmark for testing long term dependency learning, used by most work in this area. We do not add any elements to indicate the end of each row of pixels.

% In the upcoming sections, we also extend this approach by adding more challenging datasets to push the sequence lengths being tested from around 1,000 up to over 16,000 long. This limit is over 20 times longer than any previously used benchmark of this flavor. A summary of all datasets being used is presented in Table~\ref{image_statistics}.
...
\vskip -0.1in
\end{table}
\footnotetext{We follow the procedure suggested in \citet{SermanetFR14} to obtain a larger training set, while keeping the same test set. All images are scaled down to 8 different sizes from 40$\times$40 to 128$\times$128 before being flattened into sequences of pixels.}

% We first evaluate all models on the MNIST dataset and its harder version, pMNIST, where each pixel sequence is permuted. Permuting pixels breaks apart all local structures and creates even more complex dependencies across various time scales. Moving on to longer sequences, in order to perform control experiments with several scales of sequence length, we use the StanfordDogs dataset. Its training data contains large images, each categorized to one of 120 dog breeds. All images are resized down to 8 different sizes before being flattened into sequences of pixels.
...
% To illustrate the scalability of our methods, we keep subsequence length $l$ and gradients truncation fixed while varying the length of input sequences. For the experiments in this section, we choose the simplest setting of sampling frequency $n{=}1$.
% In Section~\ref{sec:sample_frequency}, we will explore different values for $n$ and $l$. With the final target of over 16,000 input tokens, we truncate supervised BPTT 
% to only 300 time steps and build the auxiliary losses over a segment of length $l{=}600$. To keep the total cost of back-propagation constant, we also truncate unsupervised gradients (backpropagated onto the main classification LSTM) to 300 time steps.\footnote{All models are implemented in TensorFlow~\cite{tensorflow2015-whitepaper}. Truncated gradients are achieved using the built-in \texttt{tf.stop\_gradient} op.}


% Thang: TODO(thtrieu) check the rewrite, move the mention of T-DCMA to later
...
% As a complement to results from purely recurrent models, we also compare our models with Transformer and one of its variants, Transformer-DMCA (T-DMCA)~\cite{liu18}
% in section~\ref{sec:transformer}. Transformer is a completely different paradigm of processing sequences that sidesteps the difficulties of BPTT. This is achieved at the cost of $O(n)$ working memory during training and inference compared to $O(1)$ for RNNs. Even though our main interest is to show improvements on recurrent models, we include these results to study how scalable the attention mechanism of Transformer is. 

\subsection{Main results}

% Thang: rename
\subsubsection{MNIST, pMNIST, and CIFAR10}
% \subsubsection{Classifying MNIST and CIFAR10}
...
% \subsubsection{Classifying Pixel-by-pixel StanfordDogs}
%Control experiment on input sequence length with StanfordDogs}
