...
% I was getting a strange error and this points to a problem with hyperref, so I disabled
% it for now -- Matt
...
\def\aclpaperid{561} %  Enter the acl Paper ID here
...
%\newcommand\wadelete[1]{\textcolor{light}{[#1]}}
\newcommand\wadelete[1]{}
%\newcommand\waadd[1]{\textcolor{dark}{[#1]}}
%\newcommand\waadd[1]{\textcolor{black}{#1}}
\newcommand\waadd[1]{#1}
...
%\author{Matthew E. Peters \\
%    Allen Institute for Artificial Intelligence \\
%    {\tt matthewp@allenai.org} \\\And
%    Waleed Ammar \\
%    Allen Institute for Artificial Intelligence \\
%    {\tt waleeda@allenai.org} \\\And
%    Chandra Bhagavatula \\
%    Allen Institute for Artificial Intelligence \\
%    {\tt chandrab@allenai.org} \\\And
%    Russell Power \\
%    Allen Institute for Artificial Intelligence \\
%    {\tt russellp@allenai.org} \\
%}
...
\maketitle
\begin{abstract}
%We introduce a method for transferring compositional functions from bidirectional
%language models trained on unlabeled text to supervised sequence tagging tasks.
...
%\waadd{The prospects of using large unlabeled data to augment labeled data used to train NLP models led to important developments in semi-supervised learning methods. %\citep[e.g.,][]{ando:05,suzuki:08,mann:10}.
%However, most semi-supervised learning methods are rarely used in practice because they add considerable constraints to the model design and training procedure, as discussed in \citet{turian:10}.}
...
%\waadd{One notable exception is the use of word embeddings, pretrained on a large unlabeled corpus, to extend the token representation in a neural network.
%Due to the simplicity and efficacy of this approach, pre-trained word embeddings have become a standard component in neural sequence tagging models.
%Our goal is to develop another simple and effective approach to learn a high quality context encoder from unlabeled data and use it to improve sequence tagging models.}
%To take advantage of much larger unlabeled corpora, many prior studies have shown that
%pre-training word embeddings from unlabeled text captures useful semantic and syntactic information
%\citep{word2vec,Pennington2014GloveGV} and including these embeddings in NLP systems has been shown


%Sequence tagging is a fundamental NLP problem and tasks such as named entity recognition (NER), part of speech (POS) tagging or shallow parsing are important components of many language understanding systems. 
%Current state of the art sequence tagging models typically share a common architecture that consists of a token level representation and a bidirectional recurrent neural network (RNN) that encodes token sequences into a context sensitive representation \citep{yang-transfer-iclr07,Ma2016EndtoendSL,lample-EtAl:2016:N16-1,joint-many-iclr07}.
%\wadelete{In many cases, model complexity is hampered by relatively small annotated training sets, particularly so for more specialized tasks such as entity extraction or POS tagging from narrow domains (see for example \citet{yang-transfer-iclr07} where the number of training tokens is typically less then 400,000).
%To take advantage of much larger unlabeled corpora, many prior studies have shown that
%%pre-training word embeddings from unlabeled text captures useful semantic and syntactic information
%\citep{word2vec,Pennington2014GloveGV} and including these embeddings in NLP systems has been shown
%to be enormously helpful for a variety of downstream tasks \citep{NLPfromScratch:Collobert2011}.}
...
%\wadelete{Neural language models (LM) capture long range context through the top layer RNN embeddings.  Recent studies \citep{Jzefowicz2016ExploringTL} have shown that it is possible to train a LM on a large-scale corpora with several orders of magnitude more tokens then typical annotated sequence tagging datasets.  }
...
%\wadelete{In much the same way that bidirectional RNNs have been shown to be superior to single direction RNNs for representing context, we employ a bidirectional LM where the backward direction operates in an analogous manner to the forward direction, but on the tokens in reverse. } 
...
%\wadelete{We apply our system to two well benchmarked sequence labeling tasks, NER and syntactic chunking.
%In both cases, we outperform previous methods, including those that use other forms of transfer or joint learning and task specific gazetteers.  Moreover, we used the same pre-trained bi-LM for
%both tasks, demonstrating that task specific pre-training is not necessary.}
...
%Fig. \ref{overview-figure} illustrates the architecture of TagLM in more detail. 
%Our formulation couples two models that share similar architectures: a neural sequence model (left side of Fig.~\ref{overview-figure}) and a pre-trained, bidirectional neural language model (right side).
%Starting at the bottom left corner of Fig.~\ref{overview-figure}, the sequence model first constructs a token representation then feeds it into the first bidirectional
%RNN layer. 
%Then, the outputs of the top layer LM in both directions are concatenated
%with the
%output of the first RNN layer in the sequence tagger, before feeding into the second RNN layer. 
%The output of the second RNN layer feeds into a dense layer followed by a linear-chain conditional random field %(CRF) layer, before making label predictions for each token in the input.

%Context sensitive representations, as captured in the output embeddings from the top layer of the bidirectional LM, are directly included in the sequence tagging model.
%Broadly speaking, TagLM first forms a representation for each token then uses a stacked bidirectional
%RNN to produce context sensitive embeddings in the sequence layer.
%Finally, the output embeddings from the top layer RNN are used
%to predict token labels with a dense layer and conditional random field (CRF).
...
%%%%%%%   OVERALL CoNLL-2003 results WITHOUT additional data
\begin{table}[t]
\begin{center}
\begin{tabular}{l|l}
\hline \hline
...
\end{table}
%%%%%%%   OVERALL CoNLL-2003 results


%%%%%%%   OVERALL CoNLL-2000 WITHOUT ADDITIONAL DATA results
\begin{table}[t]
\begin{center}
\begin{tabular}{l|l}
\hline \hline
...
\end{table}
%%%%%%%   OVERALL CoNLL-2000 results


%%%%%%%   CoNLL-2003 with additional data
\begin{table*}[t]
\begin{center}
...
%%%%%%%   OVERALL CoNLL-2000 results WITH ADDITIONAL DATA
\begin{table*}[t]
\begin{center}
\begin{tabular}{l|p{2.4in}|l|l|l}
\hline \hline
...
%%%%%%%   CoNLL-2003 COMPARISON OF ARCHITECTURE CHOICES
\begin{table}[t]
\begin{center}
\begin{tabular}{l|l}
\hline \hline
...
%%%%%%%   CoNLL-2003 COMPARISON OF LANGUAGE MODEL SIZES
\begin{table*}[t]
\begin{center}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%  OVERALL RESULTS FOR CONLL-2003 and 2000
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%  SENSITIVITY TO DIFFERENT LAYERS
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Analysis}
...
%(i.e., concatenating the LM embeddings with the output of the first RNN layer)
performs best.
...
%If the LM embeddings are instead concatenated to the top layer output (by
%replacing (\ref{eqn:rnn2}) with $\mathbf{h}_k^2 = [\overrightarrow{\mathbf{h}}_k^2, \overleftarrow{\mathbf{h}}_k^2, \mathbf{h}_k^{LM}]$), $F_1$ decreases on average by 0.21\%.
%Finally, including the LM embeddings at the token representation level (by replacing (\ref{eqn:token_rep}) with $\mathbf{x}_k = [\mathbf{c}_k, \mathbf{w}_k, \mathbf{h}_k^{LM}]$) decreases $F_1$ over both cases where the LM embeddings are included at a higher levels.
...
%\wacomment{discuss \url{http://papers.nips.cc/paper/5949-semi-supervised-sequence-learning.pdf}, %\url{https://u.cs.biu.ac.il/~melamuo/publications/context2vec_conll16.pdf}}
...
%Bidirectional language models have also been used for machine translation \citep{peris:15} and handwriting recognition \citep{frinken:12}.
...
% If we have room, we should add the following suggestions for future work before the conclusion.
%As our approach is broadly applicable to any NLP task that produces token-in-context
%embeddings, we hope our results will encourage others to explore ways to apply it to additional NLP tasks.
%Combining LM embeddings with other types of joint or transfer training
%may also prove useful, especially in cases of small annotated data sets.
%Finally, fine tuning the LM embeddings on the task specific labeled data by backpropagating tagging errors
%into the LM weights or pre-training the LM on both unlabeled and labeled data are additional
%possible avenues for future work.


%\begin{itemize}
%\item Fine tune LM embeddings on target task or jointly train the LM embeddings on the target task
%\item Instead of a separate LM, just have a joint loss function that outputs either next/previous token or sequence tag
%\item Combine with other types of transfer learning
%\item Explore whether this approach works for other tasks: parsing, or sentence level tasks (sentence similarity, etc).
%\end{itemize}
