...
% One critical functionality for autonomous vehicles in environmental mapping is to obtain a feature map by leveraging the flexible mobility of the robotic platform and the measurement capability of sensors equipped on the vehicle. The obtained feature map not only feeds back the interested values to human operators, but also is crucial for subsequent planning and decision making. In this case, an accurate and fast map prediction is necessary for the robot to quickly update the global feature map in real-time. Almost all the sensors we use can be regarded inherently inaccurate due to many error factors in designing and manufacturing, and this can lead to severely degraded sensing quality, for example, the point cloud from a depth camera may contain many outliers and the one from a low-end LiDAR~(i.e., 16-beam) might be sparse. In addition, nearly all sensors have limited sensing range, which may result in large portions of the environment unobserved/missed by the robot. 
% %The limitations on sensors produce many impediments for obtaining a desired feature map, such as measurement noise and data missingness. 
% Hence the map prediction must base itself on those partially sampled~(observed) information to quickly predict an accurate map. The problem becomes even more challenging in large-scale scenarios, which are frequently encountered in most real applications.
...
We conduct exhaustive simulations on a set of maze-like complex maps and quantitatively analyze that our proposed LRMC method is superior to BHM on the maps with varying linear patterns and different rank values. %in terms of mapping accuracy and computation time. you will repeat this immediately below.
...
% We first illustrate how a partially sampled urban building layout map fits into the LRMC model. Then we demonstrate that our proposed LRMC map prediction outperforms state-of-the-art map prediction~(or regression) method --- Bayesian Hilbert Mapping~(BHM)~\cite{senanayake2017bayesian} in terms of mapping accuracy and computation time performance. BHM is a Bayesian extension of Hilbert Mapping~(HM)~\cite{ramos2016hilbert}, where the map prediction is treated as a kernelized logistic regression problem. BHM eliminates crucial regularization parameter tunning in HM, and outputs continuous maps where the occupancy value of any arbitrary point could be queried.

% Our experiments show that BHM is incompetent in real-time applications but LRMC is able to maintain and update a global map very quickly. Finally we adopt some representative coverage planning methods commonly used for environmental mapping and show that with our proposed real-time map prediction, the coverage convergence could be significantly improved without loss of map accuracy. 
