...
\author{Lin Li$^{1}$, Xin Kong$^{1}$, Xiangrui Zhao$^{1}$, Tianxin Huang$^{1}$ and Yong Liu$^{1,*}$% <-this % stops a space
...
\begin{itemize}
    ...
    % \item We proposed a Fast Yaw Angle Calculate method to obtain the azimuth angle for the point clouds.
    % \item We proposed a Fast Semantic ICP approach to get the translation of the clouds. The 3D pose is used to align the clouds to eliminate the influence of rotation and translation on the similarity of the descriptors.
...
\begin{figure}[t]
    \centering
        % \subfigure[Origin]{
        % \centering
        % \includegraphics[width=0.4\columnwidth]{angle_origin.png}
        % }
...
% In Section \ref{ssc:yaw}, 
% we divide the input point clouds into \(N_a\) sectors and keep only one point in each sector. Through this operation, 
% we get two point clouds each containing \(N_a\) points. Note that these two point clouds are in order, as the points are sorted by yaw angle. We mark these two point clouds as \(P_{I1}\) and \(P_{I2}\). 
...
\begin{figure*}[htb]
 ...
 \end{figure*}
  ...
  \begin{table*}[htb]\footnotesize
    ...
    \begin{tablenotes} 
        % \fontsize{6.0pt}{\baselineskip}\selectfont
        \footnotesize
        ...
        % \item The SGPR is trained with grount-truth semantic label.
        % \item The OverlapNet are trained on sequences 03-10, so sequences 05, 06, 07 and 08 are included in the training set. 
     \end{tablenotes}
    \end{threeparttable}
    \end{center}
    \end{table*}
...
Fig.~\ref{pic:ssc} shows Semantic Scan Context creation. 
    ...
    \begin{figure*}[htb]
        \centering
            \subfigure[Average \(F_1\) max scores]{
            \centering
            \includegraphics[width=0.95\columnwidth]{F1.png}
            }
            \subfigure[Average \(EP\)]{
            \centering
...
% \begin{table}[htb]\footnotesize
%     \caption{\centering Yaw error on KITTI dataset}
%     \label{table:yaw}
%     \begin{center}
%     \begin{threeparttable}
%         {
...
%     Methods & 00 & 02 & 05 & 06 & 07 &08&Mean\\ 
%     \hline
%     SC  (deg)& 11.526 & 11.301 &18.394&4.074&21.862&49.17&19.388\\
%     ISC (deg)&\textbf{0.829}&1.343&0.904&\textbf{0.534}&0.684&3.856&1.358\\
%     ON  (deg)&2.595&4.911&3.329&1.124&2.233&68.622&13.802\\
%     Ours-SK (deg)&0.891&\textbf{1.142}&\textbf{0.653}&0.759&\textbf{0.512}&\textbf{1.878}&\textbf{0.973}\\
%     \hline
%     \end{tabular}
%     }
%     \begin{tablenotes} 
...
%     \end{table}
    ...
    % \begin{table}[htb]\footnotesize
    %     \caption{\centering Translation error on KITTI dataset}
    %     \label{table:kitti_result_absolute}
    %     \begin{center}
    %     \begin{threeparttable}
    %         {
    %     \begin{tabular}{c c c c c c c c}
    %     \hline
    %     Axis & 00 & 02 & 05 & 06 & 07 &08&Mean\\ 
    %     \hline
    %     x (m)&0.450&0.927&0.469&0.302&0.308&1.125&0.597\\
    %     y (m)&0.381&0.498&0.110&0.063&0.208&0.515&0.296\\
    %     \hline
    %     \end{tabular}
    %     }
    %     \end{threeparttable}
