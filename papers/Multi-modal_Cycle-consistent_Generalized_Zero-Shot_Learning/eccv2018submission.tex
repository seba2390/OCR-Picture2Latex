...
\pagestyle{headings}
\mainmatter
\def\ECCV18SubNumber{2156}  % Insert your submission number here
...
%
%
Since our approach is based on the f-CLSWGAN~\cite{XianCVPR2018}, we re-implemented this methodology.%~\footnote{Please note that we requested a copy of the f-CLSWGAN code~\cite{XianCVPR2018} from the authors, but we were not successful at receiving such copy, so we re-implemented it and made sure we achieved a similar result to the ones reported in~\cite{XianCVPR2018} -- please see Tab.~\ref{table:lit_baseline} comparing our implementation (\textbf{baseline}) with published results.}.  
...
%%%%%%%%%%%%%%%%%%%%%%%%%
% Input: GZSL-TABLE
\input{src/gzsl_table.tex}
%%%%%%%%%%%%%%%%%%%%%%%%%

%%%%%%%%%%%%%%%%%%%%%%%%%
% Input: ZSL-TABLE
\input{src/zsl_table.tex}
%%%%%%%%%%%%%%%%%%%%%%%%%
