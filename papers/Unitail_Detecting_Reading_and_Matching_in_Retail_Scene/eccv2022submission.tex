...
\pagestyle{headings}
\mainmatter
\def\ECCVSubNumber{1259}  % Insert your submission number here

\title{Unitail: Detecting, Reading, and Matching in Retail Scene} % Replace with your title
...
SPR refers to the automatic detection and recognition of products in complex retail scenes. It comprises steps that first localize products and then recognize them via the localized appearance, analogous to many recognition tasks. However, scene products have their characteristics: \textit{they are densely-packed, low-shot, fine-grained, and widely-categorized}. These innate characteristics result in obvious challenges and will be a continuing problem. Recent datasets in retail scenes follow the typical setting in the common scene to initiate the momentum of research in SPR. For instance, the SKU110k dataset \cite{SKU110k}, which has recently enabled large-scale product detection, is following the MS-COCO \cite{lin2014microsoft} style annotation and evaluation metric. Despite their significant value, the underpaid attention to the SPR's characteristics leads us to the question: %what is missing in the existing datasets that impedes our continuing advance towards SPR?
what is the next advance towards SPR?
%where the later step is usually operated by matching products to an open-set product gallery. The performance of SPR is highly reliance of the datasets. Current detection datasets and recognition datasets in retail scene are following in footsteps of common scene \cite{} to help us explore the first challenges. For instances the SKU110k \cite{}, which has recently enabled large scale product detection, is following the COCO \cite{} style labeling and evaluation metric. Despite their significant practical and research value, they pay less attention to the particularity of SPR. We summarize four characteristics of scene products: \textit{they are densely-packed, low-shot, fine-grained, and widely-categorized}. These innate characteristics result in obvious challenges and will be a continuing problem towards SPR. Considering these characteristics, we focus on two critical issues that are neglected in existing benchmarks.
...
%The performance of SPR is highly reliance of two critical questions: 1. Is the product appearance properly aligned by the product detector? Well-aligned appearances exclude utmost noisy context and maintain enough representative pixels for feature extraction. 2. Is the discriminative feature abundantly extracted by the recognizer? Product recognition is fine-grained where similar products might differ slightly, so sufficient features are required for distinction. 

%Existing datasets follow in footsteps of common scene \cite{} and the particularity of retail scene are missed. For instances the SKU110k \cite{}, which has recently enabled large scale product detection, follow the COCO \cite{} style labeling and evaluation metric. 
%Thus, the two questions are neglected \cite{} and impede the continuing advance in SPR, leading to two major drawbacks:

%First, detection targets mismatch the actual needs, causing unsatisfactory alignment for product appearances. Detection targets in common scene \cite{coco, voc, widerface} are usually defined to cover the utmost visible entirety with minimal rectangles, which are inherited by existing retail datasets \cite{} and methods \cite{}. However, because of the mutual occlusion, rotation, distortion, and restricted shooting angles in retail scenarios, these detection targets suffer from drawing proper bounding boxes to align the product appearances. As shown in Fig \ref{}, axis-aligned bounding boxes(AABB) and rotated bounding boxes (RBOX) imprecisely extract the products, leading to lower matching scores. As shown in Fig \ref{}, the visible entirety is inconsistent from the same products, leads extra difficulties to detection and classification.

%Second, even if the product is well aligned, visual features of product appearance are not abundantly extracted. This is due to the intrinsic constraint of SPR task.
%On one hand, product recognition is a low-shot training task. The samples per category is scarce and the training easily overfits.
%One the other hand, product recognition is a fine-grained task. The minimal variance on product appearance might indicates a great change on category and price, especially in the text region like net weight, count, flavor, function, and ingredient (see Fig. \ref{}). This requires robust features to represent these special text patterns, but empirically, we observe that local text details are overlooked by classifiers due to the training constraint.
%Therefore, ideal features are insufficiently extracted and lead failure to finer categorization. 

%Third, significant domain shift exists among stores, while the evaluation on model generality is neglected in existing benchmarks. The domain shift mainly gives rise to false negatives of detection. It comes from diverse product categories(some stores only sell grocery while some focus on pharmacy), different contexts (shelf, price tag), and illumination variance. A product detector succeeding one domain may not, in other unseen stores, be optimal to deploy. Thus, a realistic evaluation setting is essential for academia and enterprises.
...
%Unitail-OCR consists of a gallery and a group of testing images, where on-product text locations and contents are annotated together with their class labels. These images are collected from the Unitail-Det and are benefited from the quadrilaterals. To the best of our knowledge, it is the first dataset to support OCR models' training and evaluation in this domain, and it is experimentally verified to fill in the domain blank; when evaluating on a wide variety of product texts, text models trained on Unitail-OCR performs better than those trained on common scene texts \cite{icdat15}. 
...
%Specifically, robots usually take bright pictures from the bottom up angle through high-quality cameras and build-in light source, shaping most products into trapezoid; fixed cameras are mounted on the ceiling in most cases, creating low-resolution images from top to bottom; customers prefer to photograph with mobile devices from the frontal or side-view angles, shaping products into rhomboid. The product categories sold in different stores also show a great deal of variety. 
...
%These images own three properties: 1. They are collected from a limited number of stores worldwide ($\textless 5$ based on our statistics).  2. All images are shot by holding mobile phones from side or frontal perspectives. 3. The diversity of categories is rich but still highly limited. 
...
% Past matching algorithms mainly focus on refining image-level features. In our proposed dataset, we explore the possibility of leveraging both visual and textual information. 
...
%\subsubsection{Product Detection Evaluation Metric} 
...
%The AP50 (AP under the threshold of .50) and AP75 (AP under the threshold of .75) are also reported.
...
%\subsubsection{Text Detection Evaluation Metric} 
...
%\subsubsection{Text Recognition Evaluation Metric} 
...
%The SAPD refers to the soft anchor point detector \cite{zhu2020soft} that adopts the same structure. During training, it proposes two strategies to re-weight the losses of feature pixels, boosting performance on generic objects. The first strategy by down-weighting the loss of each feature pixel by its \textit{centerness}. The centerness is proposed by FCOS\cite{tian2019fcos} to value how far a pixel locates from the center of a ground-truth: the farther, the more likely it is to predict an inaccurate box, and the harder SAPD down-weights its losses.
%The second strategy is termed \textit{Soft Selection} that down-weights losses across $n$ pyramid levels: it trains an auxiliary network that simultaneously predicts $n$ down-weighting factors for the loss of each pyramid level.
...
Table \ref{tab:detection_benchmark} shows their performances. Overall, regression based methods outperform segmentation based methods because most products can be well aligned by quadrilaterals, and the learning on segmentation mask involves extra difficulties. The RetailDet outperforms other detectors by a large margin. All detectors achieve degraded results in the cross-domain as opposed to the origin-domain, confirming that domain shift exists among stores. % The experiments across various backbone indicate that deeper model owns better generalization ability.

\begin{table}
\centering
\caption{Benchmarking product detection on the Unitail. All methods are trained and tested under same setting. g-mAP is the geometric mean of mAPs. %Retail $++$ is an enhanced variant where a box refinement module is added (see supplementary for details).
}
...
%11&RetailDet ${++}$   &ResNext101&59.2      & 61.3 & 93.1 & 71.8   & 57.1 & 84.4 & 64.4 \\
\hline
\end{tabular}
