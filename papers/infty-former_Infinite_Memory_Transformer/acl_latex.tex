...
%\newcommand{\andre}[1]{{\textcolor{red}{\bf [{\sc AM:} #1]}}}
%\newcommand{\pedro}[1]{{\textcolor{cyan}{\bf [{\sc PM:} #1]}}}
...
\begin{equation}\label{eq:continuous_embeddings}
    \bar{X}(t)=B^\top \psi(t),
\end{equation}
where ${\psi(t) \in \mathbb{R}^N}$ is a vector of $N$ RBFs, e.g., $\psi_j(t) = \mathcal{N}(t; \mu_j, \sigma_j)$, with $\mu_j \in [0,1]$, and ${B \in \mathbb{R}^{N\times e}}$ is a coefficient matrix. $B$ is obtained with %a 
multivariate ridge regression %criterion 
