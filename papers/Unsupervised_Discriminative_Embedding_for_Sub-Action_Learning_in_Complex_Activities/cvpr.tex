...
% \newcommand{\MS}[1]{{\color{blue} {\bf MS:} #1}}
% \newcommand{\YSR}[1]{{\color{red} {\bf YSR:} #1}}
% \newcommand{\hkc}[1]{{\color{magenta} {\bf HK:} #1}}
...
% {\tt\small sirnam.swetha@knights.ucf.edu, kuehne@ibm.com, {yogesh ,shah}@crcv.ucf.edu}
...
% Also in context of analysing untrimmed and complex video sequences  
...
% Overall, we obtain an accuracy gain of $5.8\%$ on Breakfast dataset, $4.8\%$ on YouTube Instructions dataset, and $6.7\%$ on 50Salads dataset. 
...
\subsection{Overview}
\label{sec:overview}
\par Given a set of $N$ videos, $\{V_{n}\}_{n=1}^{N}$, for a complex activity, we divide each video into segments and for each segment we extract I3D features~\cite{rl-a-carreira2017quo} % at regular intervals \MS{this is ambiguous, just say you divide video into segments, and compute features for each segment}
, and compute positional encoding vectors ~\cite{vaswani2017attention} as described in Section~\ref{sec:jvtl}. %\hkc{TODO: Check for consistency with section/Sec. fig. and table.. should all follow the same rule - done}
Each video 
%$V_{n}= \{\textsc{x}_{nm}\}_{m=1}^{M_{n}}$
%\MS{(I have commented the Vn here, it breaks the flow, and it also does not contain position encoding)} 
...
Thus eliminating the need for all the data to be available at once, our model can learn the latent concepts incrementally. %We show the impact of available data on model performance in Section~\ref{sec:online_learning}. %Figure~\ref{fig:bf_online}.
...
% Finally, we describe the background modeling scheme to handle the background clips. 
...
We employ an encoder-decoder %\MS{use some other term; this is too simple}  
...
% \label{eq:dsct}
    ...
    % P( k | Z_{nm}) &=& sim(Z_{nm}, Y_{k}), \label{eq:prc} \\
    % k^{*} &=& {\operatorname*{argmax}_{k}} P( k | Z_{nm}), \label{eq:sk}, 
\end{equation}
\vspace{-0.3cm}
...
% \hkc{This might fit better in the evaluation part }We call this as initial predictions and the accuracy of initial predictions is suffixed with `w/o Viterbi'. To demonstrate the robustness of our embedding, we compare MoF (Mean over Frames) \& MoC (Mean over Class) w/o Viterbi of our method with~\cite{kukleva2019unsupervised} in section~\ref{sec:exp}.

\noindent{\bf Transition modeling and Viterbi decoding}\\
~\label{sec:tmvb}
%\noindent{\bf Sub-Action Transitions}\\
...
\label{eq:state-trans}
    ...
    \end{dcases}
\end{equation}
% \MS{Any justification why you picked these .5 and 1 numbers? - following \cite{kukleva2019unsupervised}}
\paragraph{Decoding}
...
% \MS{I always have trouble in understanding this section, let us discuss.}
...
% Breakfast~\cite{bf-Kuehne12}, YouTube Instructional Videos~\cite{Alayrac16unsupervised}, and 50Salads~\cite{50s-stein2013combining} datasets. 
...
% The activities include changing car tire, cpr, jumping car, making coffee and potting a plant. 
These videos are taken from YouTube directly and have background segments where there is no sub-action.  The frequency and spread of background varies based on activity as well as on the person performing the task. Hence, the background segments neither have similar appearance nor have a temporal ordering. Therefore the background segments would be assigned to the latent concepts with very less confidence probability. Following protocol in ~\cite{kukleva2019unsupervised}, we consider $\tau$ percent of clips with least confidence as background. Only the foreground labeled segments along with latent concepts assignments form our initial predictions. We report results for background ratio of 60\%. % and the effect of background ratio on performance is included in supplementary.
% To model background, for each  cluster a probability threshold $\tau$ defines the percentage of clips to be considered as the background. At the end, we only retain the confident $1 - \tau$ percent clips which are then labeled as foreground and rest of them are marked as background.  %We report results for background ratio of 60\% on YouTube Instructions dataset, the effect of background ratio on performance is included in supplementary.
% \par In a complex activity video composed of multiple sub-actions, the sub-actions are often not continuous. The clips where no sub-action occurs are termed as background. The frequency and spread of background varies based on activity as well as on the person performing the task. Hence, the background clips neither have similar appearance nor have a temporal ordering. As the background clips do not have similarity in our joint embedding, they would be assigned to the latent concepts with very less confidence probability. 
% \par To model background, we sort all the latent vectors assigned to each latent concept based on confidence probability. Once we have the sorted probability scores for each cluster, we compute a probability threshold $\tau$, which defines the percentage of clips to be considered as the background. At the end, we only retain the confident $1 - \tau$ percent clips which are then labeled as foreground and rest of them are marked as background. Only the foreground labeled clips along with latent concepts assignments form our initial predictions.
% \MS{This seems bit ad hoc; I do not fully understand it, let us discuss.}
\begin{figure}[tb]
\centering
...
\noindent{\textbf{Metrics}} Our model predicts a sequence of cluster labels $ \in \{1,2,...,K\}$ for each video without any correspondence %\MS{(what doe correspondence mean here?) - mapping} 
...
\noindent{\bf Effect of Loss Components.} To begin with, we first examine the influence of $Loss_{r}$ and $Loss_{d}$ on our model and the performances are presented in Table~\ref{tab:ablation_loss}. It can be seen that having all loss components leads to best performance. %It can be seen that constrastive loss ($Loss_{d}$) plays an important role and by removing it the accuracy drops by 11\%. This can be attributed to discriminative cluster learning with the help of contrastive loss that helps in better latent concept learning.
