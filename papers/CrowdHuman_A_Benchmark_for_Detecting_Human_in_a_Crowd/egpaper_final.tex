...
\title{CrowdHuman: A Benchmark for Detecting Human in a Crowd} % Replace with your title
...
% The main impact of crowd is that it causes crowd occlusion thus significantly increases the difficulty in the localization process of a detector~\cite{wang2018Repulsion}. When the detection fails, unfortunately, the techniques based on detection, such as skeleton detection~\cite{yilun2018cpn}, person tracking~\cite{mot2015} and Re-ID~\cite{xiaotong2017cvpr}, also fail. 
...
%The paper is organized as follows. The proposed CrowdHuman dataset, including its collection process and statistics specifics, is described in Section~\ref{sec:dataset}.
%We then report the results of various state-of-the-art detection frameworks on CrowdHuman as a strong reference in Section~\ref{sec:expCrowdhuman}. 
%Finally, in Section~\ref{exp:crossdataset}, we demonstrate the cross-dataset generalization results of the proposed CrowdHuman dataset on Caltech-USA and CityPersons for pedestrian detection, and on Brainwash~\cite{stewart2016end} for head detection. Thanks to the rich, large-scale variation, and high-quality CrowdHuman datasets, a framework originally designed for general object detection without any specific modification achieves state-of-the-art results on every previous benchmark. 




\section{Related Work}
% Most of previous datasets related to human detection focus on pedestrians, for that pedestrian detection is at the frontier of direct and promising application scenarios, such as autonomous cars and smart surveillance. 
...
% Instead, we collect images from the Internet, remove images which do not contain many persons, and exhaustively annotate every person that appears in the dataset. Details can be found in Section~\ref{sec:dataset_statistics}.
...
\paragraph{Diversity.}
Diversity is an important factor of a dataset. %When the illumination, background, viewpoint and human poses are biased, the dataset could not have good transferability. 
...
% Occlusion can further be divided into two types: \emph{inter-class occlusion}, which occurs when a person is occluded by stuff or objects from other categories; and \emph{intra-class occlusion}, also referred to as \emph{crowd occlusion}, which occurs when a person is occluded by other persons. As described in Section~\ref{sec:dataset}, CrowdHuman dataset is collected by filtering the images without any crowd occlusion. Therefore, 
...
% We use these two architectures not only because their prevalence and strong performance, but also their generality: Faster R-CNN and RetinaNet are masterpieces of two-stage detection framework and one-stage detection framework, respectively, so our observations should remain effective when the CNN-based frameworks are modified. As we will demonstrate later in this section, with almost no modifications, with even a relatively weak baseline, our detectors can improve the state-of-the-art performance on all previous human detection benchmarks and datasets with the help of CrowdHuman.
...
% When evaluating, ignored regions that are not annotated are not considered as missing detections, while those regions that are detected are not considered as neither true positives or false positives.
...
\myparagraph{Full Body Detection} % Knowing where the full body in occlusion scene is necessary in the applications like serving robots and self-driving cars.%  In these cases, the machine should predict where the whole body is based on what it has seen to ensure the security of human body. Also, full body detection is usually important for the person re-identification where bounding-box alignment is important for feature extraction. However, it 
...
\myparagraph{Caltech and CityPersons} Caltech and CityPersons are widely used benchmarks for pedestrian detection, both of them are usually adopted to evaluate full body detection algorithms. % Thus, Caltech and CityPersons are utilized to validate the generalization ability of our CrowdHuman dataset on the full body detection.
