...
\begin{document}
\begin{sloppypar}	
%\title{Achieving High Max-Min Throughput in IRS-Aided Overloaded MISO SWIPT Systems Via User Grouping}
%\title{User Grouping for Overloaded SWIPT Systems Aided by Intelligent Reflecting Surface with Phase Errors}
%\vspace{-5mm}
%\title{IRS-Aided Overloaded Multi-antenna Systems: Joint User Grouping and Resource Allocation} 
%\title{Overloaded Multi-antenna Systems Aided by Intelligent Reflecting Surface: Joint User Grouping and Beamforming?} 
%\title{Overloaded Multi-antenna Systems Aided by Intelligent Reflecting Surface: Joint User Grouping and Resource Allocation} 
\title{IRS-Aided Overloaded Multi-Antenna Systems: Joint User Grouping and Resource Allocation} 
%\title{Joint User Grouping and Resource Allocation for IRS-Aided Overloaded Multi-Antenna Systems} 
...
\begin{abstract}
	\vspace{-2.5mm}
	%This paper considers simultaneous wireless information and power transfer (SWIPT) in a multiuser downlink system assisted by an intelligent reflecting surface (IRS) with phase errors. We explicitly concentrate on overloaded scenarios where the number of single-antenna information users (IUs) and energy users (EUs) is larger than the number of transmit antennas at the access point (AP). 
...
\vspace{-1mm}
%The limited lifespan of battery-powered wireless devices is always a major challenge in rolling out reliable and sustainable wireless networks. 
...
% are becoming ever more commonplace %  widespread
% due to the growing demands for ultra-high connectivity
% Overloaded scenarios are becoming more and more relevant in the light of the growing demands for ultralow latency and ultra-high connectivity

% The ability to simultaneously support a tremendous number of devices with heterogeneous demands and capabilities is amongst the various features envisioned for future wireless networks [7]. Hence, it is expected that many networks will operate in overloaded regimes, roughly described as scenarios where the number of messages exceeds the number of transmitting antennas.
% Initially, the multiantenna BS uses spatial multiplexing to transmit the downlink signal containing independent messages for different mobile users to a multiantenna RS, which then applies linear spatial processing to the received signal.
...
\begin{itemize}
	\item Unlike existing works (e.g., \cite{2020_Qingqing_SWIPT_letter,2020_Qingqing_SWIPT_QoS,2020_Cunhua_SWIPT,2020_Wei_SWIPT_secure,2021_Shayan_SWIPT}) where all the IUs are served simultaneously, we propose two UG schemes, namely, the non-overlapping UG scheme and the overlapping UG scheme, to assign the IUs into several groups. The second scheme is a super-scheme of the first one, distinguishing itself by allowing each IU to be assigned into multiple groups. %The second scheme differs from the first one in the sense that each IU can be assigned into more than one group. 
	...
	%thus greatly simplifying the solution design of (P2).
	...
	\item Numerical results verify the effectiveness of our proposed algorithms and indicate the importance of robust design for practical IRS-aided SWIPT systems with phase errors since a non-robust design ignoring the phase errors generally leads to an infeasible EH solution. Furthermore, our proposed UG strategies can achieve remarkable improvements in max-min throughput compared to the cases without UG or adopting random UG. %In addition, unless the absolute difference between $K$ and $M$ is small and the EH constraints are not stringent, the max-min throughput achieved by the non-overlapping UG scheme is comparable to that by the overlapping UG scheme. 
...
\begin{figure}[!t]
	...
	\subfigure[]{\label{fig:system_model_a}
		\includegraphics[scale=0.68]{system_model_a.eps}}
	\hspace{2mm}
	\subfigure[]{\label{fig:system_model_b}
		\includegraphics[scale=0.68]{system_model_b.eps}}%\vspace{-1.5mm}
...
This paper considers an IRS-aided overloaded multiuser MISO downlink SWIPT system consisting of an $N$-element passive IRS, %\footnote{As a preliminary study on UG for SWIPT in overloaded systems aided by an IRS with phase errors, we consider the simplest type of IRS architecture, i.e., passive IRS \cite{2019_Qingqing_Joint}. The extension to the more complex types of IRS architectures, such as active IRS \cite{2023_Ying_SWIPT}, hybrid IRS \cite{2022_Nguyen_hybrid}, and intelligent omni-surface \cite{2022_Shuhang_omni}, is left for our future work.}, 
...
\begin{figure}[!t]
	%\vspace{-3mm}
	\centering
	\includegraphics[width=0.8\textwidth]{frame_structure.eps}\vspace{-1.5mm}
	\caption{Illustration of the transmission protocol.}
	%Illustration of the frame structure of the proposed hybrid MU-LP and TDMA transmission strategy.}
...
%Since the objective function Rk [n] of the optimization problem P0 contains random variables, we take the expectation E{Rk [n]} to analyze it. Since the probability distribution of Rk [n] is difficult to obtain, it is not easy to find a closed-form solution of E{Rk [n]}. 
%It can be noted that the SNR of the signal received at Bob involves the random phase errors, which are generally unknown. In order to obtain the realistic SNR, we calculate the expectation of the SNR, which is given by
...
\begin{subequations}
	...
	\end{eqnarray}
\end{subequations}
where constraint \eqref{P1_cons:c} indicates that each EU is required to harvest at least $E$ Joule (J) energy and constraint \eqref{P1_cons:d} implies that the AP's instantaneous transmit power cannot exceed $P$. %$E$ denotes the minimum EH requirement of each EU and $P$ represents the AP's maximum instantaneous transmit power. 
...
%\section{Optimization Design for the Non-overlapping UG Scheme}
...
% Therefore, the AO method is adopted to solve the subproblems corresponding to different sets of variables iteratively. 
%For example, to maximize the received power of the user in a dead zone in Fig. 1, all elements of the IRS should set their reflection amplitude to the maximum value of one for maximum signal reflection; whereas to achieve signal/interference cancelation in Fig. 1, the refl ection amplitude of IRS elements may not necessarily be equal to the maximum value, and can be set different over the elements. 
...
%The subproblem with respect to $\left\lbrace \mathbf W_{\mathrm E,\ell},\{\tau_\ell\} \right\rbrace$ under given $\{\mathbf v_\ell\}$ is non-convex but can be equivalently transformed into the following convex SDP by simply applying the change of variables 
...
%The optimization variables {αn} have not been exposed in the current form of the nonconvex constraint (4b). 
%Let $r_{\rm E,\ell} = {\rm rank}\left(\mathbf W_{\mathrm E,\ell}\right)$, with $1 \leq r_{\rm E,\ell} \leq M$, and the eigenvalue decomposition of $\mathbf W_{\mathrm E,\ell}$ be denoted by $\mathbf W_{\mathrm E,\ell} = \mathbf U_\ell\mathbf \Sigma_\ell\mathbf U_{\ell}^H$, where $\mathbf U_{\ell}$ is the precoding matrix, and $\mathbf \Sigma_{\ell} = {\rm diag}\left(q_{\ell,1}, \cdots, q_{\ell,r_{\rm E,\ell}}\right)$ with $q_{\ell,1}, \cdots, q_{\ell,r_{\rm E,\ell}}$ denoting the non-negative eigenvalues of $\mathbf W_{\rm E, \ell}$. Then, we have $\mathbf W_{\rm E,\ell} = \sum_{m=1}^{r_{\rm E,\ell}}q_{\ell,m}\mathbf w_{\rm E,\ell,m}\mathbf w_{\rm E,\ell,m}^H$, where $\mathbf w_{\rm E,\ell,m}$ is drawn from $\mathbf U_{\ell} = \left[\mathbf w_{\rm E,\ell,1,}, \cdots, \mathbf w_{\rm E,\ell,r_{\rm E,\ell}}\right]$. 
\begin{lem}\label{lem}
	\vspace{-3mm}
...
\end{align}}%	
As a result, the optimization problem to be solved in the $t$-th iteration of SCA is given by%a locally optimal solution of problem \eqref{E_sub2} can be obtained by solving
{\setlength\abovedisplayskip{5pt}
\setlength\belowdisplayskip{5pt} 
...
In this section, we aim to solve (P1). %To facilitate the solution design, we first define $\mathbf W_{k,\ell} = \mathbf w_{k,\ell}\mathbf w_{k,\ell}^H$, $\forall k\in\mathcal K$, $\ell\in\mathcal L$, where 
...
%Recall that we let $\mathbf S_{\mathrm E,\ell} = \tau_\ell\mathbf W_{\mathrm E,\ell}$, $\forall \ell\in\mathcal L$, in the previous subsection. Similarly, we apply the change of variables $\mathbf S_{k,\ell} = \tau_\ell\mathbf W_{k,\ell}$, $\forall k\in\mathcal K$, $\ell\in\mathcal L$, we can further transform constraints \eqref{P1_cons:b_eqv}-\eqref{P1_cons:d_eqv} into
...
%\begin{align}
%\mathbf S_{k,\ell}\succeq \mathbf 0, \ {\rm rank}\left(\mathbf S_{k,\ell}\right) \leq 1, \ \forall k\in\mathcal K, \ell\in\mathcal L.
%\end{align}
...
\begin{subequations}
	...
	\end{align}
\end{subequations}
Note that \eqref{P1_cons:f_1} is a linear constraint while \eqref{P1_cons:f_2} is a reverse convex constraint that yields a disconnected feasible region. To handle \eqref{P1_cons:f_2}, %we resort to the penalty method.  
...
\end{align}%}% 
%The detailed steps of the proposed SCA-based algorithm for problem \eqref{P1_relaxed_eqv2_sub2} are similar to those of the inner layer of Algorithm \ref{Alg1} and are omitted due to the space limitation.  
...
\begin{figure}[!t]
	\centering
	\includegraphics[width=0.66\textwidth]{simulation_setup.eps}
...
%In fact, the more ERs are in the system, the more difficult it is for the transmitter to provide fair resource allocation for all ERs. In particular, for a large number of ERs in the system, it is more likely that there are some ERs with poor channel qualities. Thus, the transmitter is forced to steer the information and energy signals toward ERs with weak channel conditions which reduces the minimum harvested power per ER.

\vspace{-1mm}
\subsection{Achievable Max-Min Throughput}
This subsection compares the achievable max-min throughputs of our proposed non-overlapping and overlapping UG schemes with those of the following two benchmark schemes: 1) \textbf{Random UG}: $a_{k,\ell}$ is non-optimized and randomly selected from $\{0,1\}$, $\forall k\in\mathcal K$, $\ell\in\mathcal L$;  %satisfying $\sum_{\ell\in\mathcal L}a_{k,\ell} = 1$; 
...
\begin{figure}[!t]
	...
	\subfigure[$E = 1\times 10^{-5}$ J.]{\label{fig:R_vs_IUs_E1}
		\includegraphics[scale=0.71]{R_vs_IUs_E1.eps}}
	\hspace{-8.5mm}
	\subfigure[$E = 2\times 10^{-5}$ J.]{\label{fig:R_vs_IUs_E2}
		\includegraphics[scale=0.71]{R_vs_IUs_E2.eps}}\vspace{-1.4mm}
...
\begin{figure}[!t]
	...
	\subfigure[]{\label{fig:active_slots_vs_IUs}
		\includegraphics[scale=0.71]{active_slots_vs_IUs.eps}}
	\hspace{-8.5mm}
	\subfigure[]{\label{fig:sum_a_vs_IUs}
		\includegraphics[scale=0.71]{sum_a_vs_IUs.eps}}\vspace{-1.4mm}
...
%Nevertheless, it is worth pointing out that if there are more groups, each with fewer IUs, although more multiplexing gain can be explored in each time slot, the transmission duration allocated to each group is shorter. %more active time slots are required for the overlapping and non-overlapping UG schemes when $E = 2 \times 10^{-5}$ J than when $E = 1 \times 10^{-5}$ J. This result indicates that %It is interesting to observe from Fig.  \ref{fig:active_slots_vs_IUs} that the number of active time slots is 
%Fig. \ref{fig:active_slots_vs_IUs} shows that the number of user groups increases with $K$. 

%most or all the resource will be allocated to good users to get the maximal total harvested energy
% A max-min fairness based algorithm allows the users with poor channel conditions to occupy most of the resource
% It can be explained by the fact that 
%It is observed that the achievable EE under both non-linear and linear EH models decreases with regard to the EH requirement. The reason may be that the increment of the EH requirement induces more power consumption while contributing few to the sum rates.

%SDMA face limitations when the channel strengths are too different between the users and when the number of users increases, since it needs more transmit antennas than users to successfully deal with the multi-user interference
% This can be roughly attributed to the number of data streams exceeding the number of spatial Degrees of Freedom (DoF) created through classical beamforming.
% Overloaded regimes, described as the scenarios where the number of served users exceeds the number of transmitting antennas, are becoming more important due to the growing demands for ultra-high connectivity. Applications of overloaded scenarios can also be found in multibeam satellite systems where each beam carries the messages of multiple users, forming a multicast group [45], as well as in NOMA [38], [44], and coded caching [46].
...
\begin{figure}[!t]
	...
	\begin{minipage}[t]{0.5\linewidth}
		%\centering
		\hspace{-3mm}
		\includegraphics[scale=0.71]{R_vs_L.eps}\vspace{-1.5mm}
	...
	\begin{minipage}[t]{0.5\linewidth}
		\centering
		\includegraphics[scale=0.71]{R_vs_EUs.eps}\vspace{-1.5mm}
...
\begin{figure}[!t]
	...
	\subfigure[$E = 1\times 10^{-5}$ J.]{\label{fig:R_vs_N_E1}
		\includegraphics[scale=0.71]{R_vs_N_E1.eps}}
	\hspace{-8.5mm}
	\subfigure[$E = 2\times 10^{-5}$ J.]{\label{fig:R_vs_N_E2}
		\includegraphics[scale=0.71]{R_vs_N_E2.eps}}\vspace{-1mm}
...
%This is due to the facts that the high passive beamforming gain promised by large $N$ makes the EH requirement no longer be a limiting factor to the performance and 
%Nevertheless, for the case of $E = 1 \times 10^{-5}$ J, the max-min throughput performance of each scheme is observed to gradually reach a saturated state the performance is observed to be saturated at high power regime 
%Furthermore, the performance improvement brought by increasing $N$ is more evident with $E = 2 \times 10^{-5}$ J than with $E = 1 \times 10^{-5}$ J. We explain this result based on the following two facts. When $E = 2 \times 10^{-5}$ J, %First, a stringent EH requirement is the performance bottleneck of SWIPT systems, whereas a loose EH requirement can be easily satisfied. Second, 

%However, for higher values of the transmit power, the resulting intensified interference limits the maximum achievable data rate of users to a system-dependent value which in return results in a saturated EEIT value of the system.


%When the number of IRS elements is small, the growth rate is faster. When the number of IRS elements is too large, the improvement speed of energy efficiency will slow down, which is better than IRS, which is a passive device, and the upper limit of energy efficiency is limited by the energy of the base station.

%Furthermore, it is observed that there is a marginal gain in the latency reduction with respect to the increasing N, as large N will no longer be a limiting factor to the performance in the considered setup when the number of users is small.

\iffalse
\begin{figure}[!h]
	\centering
	\includegraphics[scale=0.71]{R_vs_EUs.eps}
	%\vspace{-3mm}
	\caption{Average max-min throughput versus the number of EUs for $K = 5$, $L = 3$, and $E = 1\times10^{-5}$ J.}
	\label{fig:R_vs_EUs}
	%\vspace{-5mm}
...
\begin{figure}[!h]
	\centering
	\includegraphics[scale=0.71]{R_vs_L.eps}
	%\vspace{-3mm}
	\caption{Average max-min throughput versus the number of time slots for $J = 8$ and $E = 1\times10^{-5}$ J.}
	\label{fig:R_vs_L}
	%\vspace{-5mm}
...
Moreover, our proposed UG schemes can remarkably improve the max-min throughput performance compared to the case without UG, as they enable higher active and passive beamforming gains by serving fewer IUs concurrently. %since higher active and passive beamforming gains can be obtained by serving fewer IUs at the same time, 
%This indicated that by grouping the IUs, higher active and passive beamforming gains can be obtained, although the duration that the AP serves each IU is reduced.  
Finally, unless the absolute difference between the number of transmit antennas at the AP and the number of IUs is small and the EH constraints are loose, the max-min throughput achieved by the non-overlapping UG scheme is comparable to that by the overlapping UG scheme. Thus, in most scenarios, the non-overlapping UG scheme is more attractive to practice systems due to its comparable performance to the overlapping UG scheme and extra advantage of easier implementation. %\looseness=-1 %if the absolute difference between the number of transmit antennas at the AP and the number of IUs is small and the EH constraints are loose, the overlapping scheme is observably superior to its non-overlapping counterpart in terms of achievable max-min throughput.  
