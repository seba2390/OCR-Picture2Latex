...
\begin{algorithmic}[1]
\Initialize{\strut$h_{0,i} \gets \vec{0}$, $e_{0,i} \gets \textit{embedding(<s>)}$, $s_{0,i} \gets 0$, $i=1,\ldots,k$ %\\ $\tilde{s}_t \in \mathbb{R}^{k\times|V|} \gets \vec{0}$, $\textit{b}_t \in \mathbb{N}^{k} \gets \vec{0}$, $y_t \in \mathbb{N}^{k} \gets \vec{0}$, $t=1,\ldots,T$
}
\For{t = 0 to T}
\For{i = 1 to k}
\ForAll{$v \in V$}
\State $\tilde{s}_t[i,v] \gets s_{t,i} + f(h_{t,i},v)$ \Comment{$f$ is the local output scoring function}
\EndFor
\EndFor
 \State $s_{t+1} \gets$ \textit{top-k-max}$(\tilde{s}_t)$ \Comment{Top \textit{k} values of the input matrix}
 \State $\textit{b}_{t+1, *}, y_{t, *} \gets$ \textit{top-k-argmax}$(\tilde{s}_t)$ \Comment{Top $k$ argmax index pairs of the input matrix}
 \For{i = 1 to k}
  \State $e_{t+1,i} \gets$ \textit{embedding}($y_{t,i}$)
  \State $h_{t+1,i} \gets r(h_{t,i},e_{t+1,i})$ \Comment{$r$ is a nonlinear recurrent function that returns state at next step}
 \EndFor
\EndFor
...
% As a result, for some tasks, cross entropy training yields models that benefit from search errors: as we show in experiments, for cross entropy trained models, greedy decoding can actually outperform beam search at test time.
...
% A common approach for training these models is to consider the target sequence prediction problem as generation of target sequences from a conditional language model that produces words step-by-step, and maximizing the log-likelihood(minimize the step-wise cross entropy) of the gold target sequences in the training data. The computation of log-likelihood of the training data decomposes across the time steps, where the probability of generating a word at each time-step depends on the input sequence and the \textit{gold} history of target words \textit{in the training data}. For target prediction after training, simple decoding methods like greedy deocding, beam search etc. are used. This method of training suffers from \textit{exposure bias} because of mismatch between training procedure and decoding procedure as the input to the language model during training relies on the availability of gold labels. Several approaches like reinforcement learning\cite{abc}, imitation learning\cite{abc} and discrete search based methods\cite{abc} have been proposed to ameliorate this issue which focus on making the optimization procedure of the model aware of the decoding procedure to be used during test time. In this work, we propose a novel training procedure that incorporates beam search into the optimization procedure itself of the seq2seq model and hence eliminate the mismatch between training and decoding procedures. To this end, we propose a continuous approximation to the beam search procedure so that optimization remains fully backproppable and is informed by the gradients involved with the originally discontinuous search procedure.

% \textit{Beam search} has typically been used during decoding to approximately find the maximum scoring target sequence given a model and generally outperforms greedy decoding as it explores the search space of target sequences more effectively. Previous work has also focused on making the optimization procedure \textit{search-aware} \cite{abc} and has shown improvements over the search agnostic baselines. However, search over discrete spaces is discontinuous and hence it is non-trivial to directly incorporate effects of search into the optimization of neural models because they rely on gradients at every step for backpropagation of the loss and proper credit assignment. To overcome this issue, models like those proposed by \citet{abc} ignore the gradient information related to the discontinuous search procedure and instead perform credit assignment by techniques like \textit{early updates} that explicitly track the reachability of the gold target sequence during the search procedure. With our continuous approximation to beam search procedure, we have instead, access to the gradients associated with the search procedure itself which can be used for appropriate credit assignment during training.

% The proposed continuous approximation to the beam search procedure allows us to experiment with continuous approximations to originally non differentiable losses like Hamming loss for the target sequence and max-margin loss. We empirically show that on two natural language processing tasks with very different output search spaces: \textit{Named Entity Recognition}(NER) and \textit{CCG supertagging}, our approach outperforms standard cross-entropy training followed by either greedy or beam search decoding.
...
%, as is done in much of the structured prediction literature \cite{taskar2004max,tsochantaridis2005large}.
.}. 
%We denoted the function that computes the result of beam search given an input sequence and a seq2seq model as $\textit{Beam}(x,\mathcal{M}(\theta))$. 
%We can write an objective function to , which 
...
% To ameliorate all of the above problems, we propose a continuous relaxation to the beam search procedure which in turn allows us to work with a surrogate continuous approximation to the discontinuous loss function which can be composed with the continuous beam search procedure itself to yield a fully continuous end-to-end training objective that is amenable to backpropagation:
% \begin{align}
% \min_{\theta} \mathcal{L}\circ\textit{Beam}(x,y^*;\mathcal{M}(\theta))
% \end{align}
...
\begin{algorithmic}[1]
\Inputs{$s \in \mathbb{R}^{k\times|V|}$}
\Outputs{$p_{i} \in \mathbb{R}^{k\times|V|}$, s.t. $\sum_j p_{ij}=1, i=1, \ldots, k$}
\State $m \in \mathbb{R}^{k} = \textit{top-k-max}(s)$
\For{$i$ = 1 to k} \Comment{\emph{peaked-softmax} will be dominated by scores closer to $m_i$}
\State $p_{i} = \textit{peaked-softmax}_{\alpha}(-(s - m_i\cdot{\mathbf{1}})^2)$ \Comment{The square operation is element-wise}
\EndFor
\end{algorithmic}
\end{algorithm*}
...
%\subsection{Training with Continuous Relaxation of Beam Search based Objective \label{sec-soft-beam}}
...
\begin{algorithmic}[1]
\Initialize{\strut$h_{0,i} \gets \vec{0}$, $e_{0,i} \gets \textit{embedding(<s>)}$, $s_{0,i} \gets 0$, $D_{t} \in \mathbb{R}^{k} \gets \vec{0}$, $i=1,\ldots,k$} %\\ $\tilde{s}_t \in \mathbb{R}^{k\times|V|} \gets \vec{0}$, $\tilde{D}_t \in \mathbb{R}^{|V|} \gets \vec{0}$, $t=1,\ldots,T$ \\ $\tilde{b}_{t,i} \in \mathbb{R}^{k} \gets \vec{0}$,$t=1,\ldots,T$, $i=1,\ldots,k$}
\For{t = 0 to T}
\ForAll{$w \in V$}
\For{i=1 to k}
\State $\tilde{s}_t[i,w] \gets s_{t,i} + f(h_{t,i},w)$ \Comment{$f$ is a local output scoring function}
\EndFor
\State $\tilde{D}_{t,w} = d(w)$ \Comment{$\tilde{D}_t$ is used to compute $D_{t+1}$}
\EndFor
 \State $p_1,\ldots,p_k \gets \textit{continuous-top-k-argmax}(\tilde{s}_t)$ \Comment{Call Algorithm 2}
 \For{i = 1 to k}
  \State $\tilde{b}_{t,i} \gets \textit{row\_sum}(p_{i})$ \Comment{Soft back pointer computation}
  \State $a_i \in \mathcal{R}^{|V|} \gets \textit{column\_sum}(p_i)$ \Comment{Contribution from vocabulary items}
  \State $e_{t+1,i} \gets a_i^{T} \times \textit E$ \Comment{Peaked distribution over the candidates to compute $e,D,S$}
  \State $D_{t+1,i} \gets a_i^{T} \cdot \tilde{D}_t$
  \State $s_{t+1,i} = \textit{sum}(\tilde{s}_t \odot p_i)$
  %\State $s_{t+1,i} \gets p_i \cdot \textit{flatten}(\tilde{s}_t)$
  ...
  \EndFor
  \State $h_{t+1,i} \gets r(\tilde{h}_{t,i},e_{t+1,i})$ \Comment{$r$ is a nonlinear recurrent function that returns state at next step}
 \EndFor
\EndFor
\State $L = \textit{peaked-softmax}_{\alpha}(s_T) \cdot D_{T}$ \Comment{Pick the loss for the sequence with highest model score on the beam in a soft manner.}
\end{algorithmic}
\end{algorithm*}
\begin{figure*}[t]
\centering
...
%Modify this to talk about pointwise convergence of 
...
%We experimented with different annealing schedules for $\alpha$ starting with non peaked softmax moving toward peaked softmax across epochs so that learning is stable with informative gradients. This is important because cost functions like Hamming distance cost with very high $\alpha$ tend to be non-smooth and are generally flat in regions far away from changepoints and have a very large gradient near the changepoints which makes optimization difficult.
\input{table_super.tex}
\input{table_ner.tex}
